...
s32 execute_mario_action(UNUSED struct Object *o) {
    ...
    if (gMarioState->bowserPoints >= pointsNeeded) {
        print_text(80, 120, "LEVEL COMPLETE");
        if (gMarioState->cutsceneTimer == 0) {
            play_music(SEQ_PLAYER_ENV, SEQUENCE_ARGS(15, 0x12), 0);
        }
        gMarioState->cutsceneTimer += 1;

        if (gMarioState->cutsceneTimer > 90) {
            gMarioState->cutsceneTimer = 0;
            gMarioState->bowserPoints = 0;
            gMarioState->spawnedObjectsTotal = 0;
            gMarioState->spawnedObjects = 0;
            switch(gCurrLevelNum) {
        case LEVEL_BOWSER_1:
            initiate_warp(LEVEL_BOWSER_2, 1, 0x0A, 0);
        break;
        case LEVEL_BOWSER_2:
